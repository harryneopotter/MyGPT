...
- If tests exist, a single command that runs them all and passes (current: `python -m pytest`).

## Current Progress (Working State)
- Phase 1: UI talks to backend only; streaming path exists.
- Phase 2: SQLite append-only message log with immutability triggers and transcript rendering.
- Phase 3: Model gateway with local model streaming via backend; stop/cancel support in place.
- Phase 4: Preference proposal pipeline (infer → propose → approve) with one-at-a-time proposals after answers.
- Phase 5: Clarifying-question gate and regenerate semantics implemented in backend. **(COMPLETE)**
- Phase 6: Tool Execution Sandbox (Structured, Inspectable). **(COMPLETE)**
- Phase 7: Performance, Packaging, and No-Background-Agents Guarantee. (In Progress)

**Latest verification**
- Tests: `python -m pytest`
- Dev deps install: `python -m pip install -r src/backend/requirements-dev.txt`

---

## Phase 0 — Repo & Toolchain Baseline (COMPLETE)
...
**Checklist (Pass/Fail)**
- [x] A single command starts backend in dev mode (e.g., `python -m uvicorn ...`).
- [x] A single command starts UI in dev mode (e.g., `npm run tauri dev`).
- [x] No network access required for core local operation (optional cloud explicitly off by default).
...
## Phase 1 — Control Plane + UI Shell (No Models Yet) (COMPLETE)
...
**Checklist (Pass/Fail)**
- [x] UI calls only backend endpoints (no direct calls to any model server URLs).
- [x] Backend exposes a single base URL for UI (dev + prod config).
- [x] Streaming path exists end-to-end (UI can render incremental tokens from a dummy generator).
...
## Phase 2 — Immutable Message Log (SQLite) + Rendering (COMPLETE)
...
**Checklist (Pass/Fail)**
- [x] No code path updates or deletes message rows (no “edit message” feature).
- [x] “Correction” is implemented as a new message referencing a prior message (e.g., `corrects_message_id`).
- [x] Backend treats stored messages as historical facts when assembling prompts.
- [x] SQLite enforces immutability with triggers that `RAISE(ABORT, ...)` on `UPDATE`/`DELETE` for message rows.
...
## Phase 3 — Model Gateway + Streaming (Local Models First) (COMPLETE)
...
**Checklist (Pass/Fail)**
- [x] UI still does not talk to model servers (only backend).
- [x] Backend can stream tokens and handle cancel cleanly (no partial DB corruption).
- [x] Prompt assembly is deterministic and traceable (log request IDs + inputs, excluding secrets).
...
## Phase 4 — Preference Inference Pipeline (Infer → Propose → Approve) (COMPLETE)
...
**Checklist (Pass/Fail)**
- [x] Inferred preferences do **not** alter behavior until approved (PRD §3.2).
- [x] Verified that inferred-but-unapproved preferences produce identical reasoning, framing, defaults, and outputs as baseline behavior.
- [x] Verified that no preference inference alters question selection or response structure prior to approval.
- [x] At most one proposal surfaces at a time (PRD §4).
- [x] Proposals never appear before an answer and never interrupt streaming (PRD §5).
- [x] Explicit user intent overrides preferences in every code path (PRD §2.2).
- [x] Preference approvals are stored as explicit events (who/what/when), not implicit flags.
...
## Phase 5 — Questioning Rules + Regeneration Semantics (COMPLETE)
...
**Checklist (Pass/Fail)**
- [x] No validation/permission-seeking questions (“Do you want me to…?”) unless user asked for options.
- [x] Regenerate does not create new preference proposals or reset existing approved preferences.
- [x] Regenerate does not silently change stored past messages.
...
**Checklist (Pass/Fail)**
- [x] No “direct shell exposure by default” (no arbitrary command execution from the model).
- [x] Every tool run is user-triggered and attributable to a specific message.
- [x] Verified that tools execute only as a direct, immediate consequence of a specific user message and never via chaining, scheduling, background execution, or speculative prefetch.
- [x] Tool outputs are stored/attached as artifacts without mutating message history.
...
## Phase 7 - Performance, Packaging, and No-Background-Agents Guarantee (COMPLETE with documented UI startup gap)
